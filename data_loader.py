#data_loader.py
import os
import pandas as pd
import glob


def load_electricity_data(dataset_config):
    """
    加载电力数据集，读取每个城市的电力情况作为独立的任务。
    """
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        df = pd.read_excel(file, sheet_name=None)  # 读取所有子表
        for sheet_name, sheet_data in df.items():
            # 获取文件名并去除路径
            filename = os.path.basename(file)  # 例如 '2018_WCMA.xlsx'
            
            # 提取年份（假设文件名格式为 '2018_WCMA.xlsx'）
            year = filename.split('_')[0]  # '2018'
            
            # 创建新的任务名（例如 '2018_WCMA'）
            sheet_name_with_year = f"{year}_{sheet_name}"  # 例如 '2018_WCMA'

            # 只选择时间列和目标列
            sheet_data = sheet_data[[time_column, target_column]]
            
            # 存储任务ID（加入年份）和数据
            data.append((sheet_name_with_year, sheet_data))
    return data


def load_temperature_data(dataset_config):
    """
    加载温度数据集，读取每个城市的温度数据作为独立的任务。
    """
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    folders = glob.glob(os.path.join(data_directory, '*'))
    for folder in folders:
        # 仅在目录下找到文件夹
        if os.path.isdir(folder):
            files = glob.glob(os.path.join(folder, '*' + file_pattern))  # 获取所有csv文件
            for file in files:
                df = pd.read_csv(file)
                df = df[[time_column, target_column]]  # 只选择时间列和目标列
                city_name = os.path.basename(file).split('.')[0]  # 使用文件名作为城市名
                data.append((city_name, df))  # 存储任务ID和数据
    return data


def load_walmart_data(dataset_config):
    """
    加载Walmart销售数据集，读取每个店铺的销售数据作为独立的任务。
    """
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        #df = pd.read_csv(file)
        df = pd.read_excel(file)
        df = df[[time_column, target_column]]  # 只选择时间列和目标列
        store_name = os.path.basename(file).split('.')[0]  # 使用文件名作为店铺名
        data.append((store_name, df))  # 存储任务ID和数据
    return data

def load_co2emission_data(dataset_config):
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        df = pd.read_excel(file)
        df = df[[time_column, target_column]]  # 只选择时间列和目标列
        store_name = os.path.basename(file).split('.')[0]  # 使用文件名作为店铺名
        data.append((store_name, df))  # 存储任务ID和数据
    return data

def load_gdp_data(dataset_config):
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        df = pd.read_excel(file)
        df = df[[time_column, target_column]]  # 只选择时间列和目标列
        store_name = os.path.basename(file).split('.')[0]  # 使用文件名作为店铺名
        data.append((store_name, df))  # 存储任务ID和数据
    return data

def load_weather_data(dataset_config):
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        df = pd.read_excel(file)
        df = df[[time_column, target_column]]  
        store_name = os.path.basename(file).split('.')[0]  
        data.append((store_name, df))  
    return data

def load_indian_data(dataset_config):
    data = []
    data_directory = dataset_config["data_directory"]
    file_pattern = dataset_config["file_pattern"]
    time_column = dataset_config["time_column"]
    target_column = dataset_config["target_column"]

    # 获取所有符合模式的文件
    files = glob.glob(os.path.join(data_directory, '*' + file_pattern))
    for file in files:
        df = pd.read_excel(file)
        df = df[[time_column, target_column]]  
        store_name = os.path.basename(file).split('.')[0]  
        data.append((store_name, df))  
    return data
